<template>
    <div class="list-header">
        ...
        <!-- <div class="buttons">
            <RouterLink :to="{ path: '/settings/backoffice-users/create' }">
    ...
    <div class="list-table">
        ...
        >
            ...
            <el-table-column label="Role" prop="authorityName" min-width="200">
                <template #default="{ row }">
                    <!-- {{ formatSplitRole(formatEnumToText(row.authorityName)) }} -->
                </template>
            </el-table-column>
            <el-table-column prop="phoneNumber" label="Phone" width="160" />
            <el-table-column prop="staffId" label="Staff ID" width="160" />
            <el-table-column prop="deptName" label="Department Name" min-width="200" />
            <el-table-column prop="branchCode" label="Branch Code" width="180" />
...
import { dateFormatter } from '@/utils/formatter.util';
...
const onLoad = async () => {
    await userStore.getList(currentPage.value, pageSize.value, '', '', inputSearch.value)
    .then(() => {
            isEmpty.value = userStore.data.length === 0
        })
        .catch((error) => {
            if (error.response && error.response.status !== 401) messageBoxUtil.error(error.toString())
        })
}
...
const indexNoColumn = (index: number) => {
    return pageSize.value * (currentPage.value ? currentPage.value - 1 : 0) + index + 1
}
